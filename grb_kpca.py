...
parser.add_option("-k", "--kernel-method", action="store", type="string", default="linear", help="Kernel Option: linear, poly, rbf, sigmoid, cosine, precomputed: Default is linear.")
parser.add_option("-g", "--gamma", action="store", type="int", default=10, help="Kernel coefficient for rbf and poly kernels. Default is 1/(n_features)")
...
kpca=KernelPCA(kernel=knl, fit_inverse_transform=True, gamma=gam)
#pca=PCA(n_components=nopax)
#X_kpca=kpca.fit_transform(X)
X_r=kpca.fit_transform(X)
#X_r=kpca.inverse_transform(X_kpca)
...
# KPCAed Data Generation (.ann input files)

g=open(pca_dir+'/'+'KPCAed_'+filename+'.ann','a')
...
print "Generating Kernel PCAed .ann Input files..."
...
print('Drawing Kernel PCA figure....')
...
plt.suptitle('KPCA of '+str(nopax)+'P_Axes GRB Data:'+filename, fontsize=9, fontweight='bold')
